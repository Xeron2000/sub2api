...
export function AnnouncementsPage() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["admin-announcements", search, pagination.pageIndex, sorting],
    queryFn: async () => {
      const res = await httpClient.get<{ items: Row[] }>("/admin/announcements", { params: { search, page: pagination.pageIndex+1, page_size: pagination.pageSize, sort: sorting[0] ? `${sorting[0].id}.${sorting[0].desc ? "desc" : "asc"}` : undefined } })
  ...
  return (
    <Page>
      <PageHeader title={t("admin.announcements.title") as string || "Announcements"} description="Announcements management." actions={<Button>Create</Button>} />
