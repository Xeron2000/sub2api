...
export function UsagePage() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["admin-usage", search, pagination.pageIndex, sorting],
    queryFn: async () => {
      const res = await httpClient.get<{ items: Row[] }>("/admin/usage", { params: { search, page: pagination.pageIndex+1, page_size: pagination.pageSize, sort: sorting[0] ? `${sorting[0].id}.${sorting[0].desc ? "desc" : "asc"}` : undefined } })
  ...
  return (
    <Page>
      <PageHeader title={t("admin.usage.title") as string || "Usage"} description="Usage management." actions={<Button>Create</Button>} />
