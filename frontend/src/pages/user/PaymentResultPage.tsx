...
export function PaymentResultPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["payment-paymentresult", orderId],
    queryFn: async () => {
  ...
  if (!orderId) return <Page><PageHeader title="PaymentResult" description="Payment flow — missing order." /><Section><Card className="rounded-none"><CardContent className="p-6 text-sm">No order_id. Start at <a href="/purchase" className="underline">Purchase</a>.</CardContent></Card></Section></Page>
  const status = (data as { status?: string })?.status ?? "loading"
  return (
    <Page>
      <PageHeader title="PaymentResult" description={`Order ${orderId} — backend-authoritative polling.`} />
      <Section>
