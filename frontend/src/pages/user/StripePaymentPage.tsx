...
export function StripePaymentPage() {
  ...
  if (!orderId) return <Page><PageHeader title="StripePayment" description="Payment flow — missing order." /><Section><Card className="rounded-none"><CardContent className="p-6 text-sm">No order_id. Start at <a href="/purchase" className="underline">Purchase</a>.</CardContent></Card></Section></Page>
  const status = (data as { status?: string })?.status ?? "loading"
  return (
    <Page>
      <PageHeader title="StripePayment" description={`Order ${orderId} — backend-authoritative polling.`} />
      <Section>
