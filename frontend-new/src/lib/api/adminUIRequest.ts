...
function isAdminPath(path: string): boolean {
  return path === "/admin" || path.startsWith("/admin/") || path === "/api/v1/admin" || path.startsWith("/api/v1/admin/")
}
...
export function isUserTimingAPIPath(requestURL: string): boolean {
  ...
  if (path.startsWith("/payment/")) {
    if (path.startsWith("/payment/public") || path.startsWith("/payment/webhook")) return false
    return true
  }
  return false
}
