...
> Generated: 2026-08-26 | Goal 4 activated: 2026-08-26 | Updated: 2026-08-26 Task1 Guard底座硬化完成
...
| Route | Feature | Old Source | Flag | Status | Notes |
|-------|---------|-----------|------|--------|-------|
| `/setup` | Setup wizard | `views/setup/SetupWizardView.vue` | — | DEFERRED_SPECIAL_FLOW | Goal 5 — setup wizard deferred |
| `/auth/callback` (+ linuxdo/wechat/dingtalk/oidc) | OAuth callback | `views/auth/*Callback*.vue` | — | DEFERRED_SPECIAL_FLOW | Goal 5 — OAuth callback deferred |
| `/payment/qrcode`, `/payment/stripe`, `/payment/airwallex`, `/payment/stripe-popup` | Payment SDK flows (QR/Stripe/Airwallex) | `views/user/PaymentQRCodeView.vue` etc | payment | DEFERRED_SPECIAL_FLOW | Goal 5 (SDK) — payment SDK deferred |
| `/custom/:id` | Custom pages | `views/user/CustomPageView.vue` | — | DEFERRED_SPECIAL_FLOW | Goal 5 — custom page renderer deferred |
| `/model-plaza` | Model plaza | `views/ModelPlazaView.vue` | — | DEFERRED_SPECIAL_FLOW | Goal 5 — model plaza deferred |
| `BackupView.vue` | Backup/restore inside Settings | `views/admin/BackupView.vue` (885L) — `api/admin/backup.ts`, `dataManagement.ts` | — | DEFERRED_SPECIAL_FLOW | Inventory only in Goal 4; full flow deferred if not standalone route (track in Notes) |

### Cross-cutting Compliance Inventory

| Concern | Old Implementation | Current React | Parity |
|---------|-------------------|---------------|--------|
| Admin Guard | `router.beforeEach`: `requiresAuth` + `requiresAdmin` + `fetchStatus` (adminCompliance) + `requiresPayment`/`requiresRiskControl` + `isSimpleMode` check + `backendModeEnabled` | `frontend-new/src/lib/guard/adminGuard.ts` unified `createAdminGuard({blockSimpleMode,requirePayment,requireRiskControl})` + `getAuthStatus` three-state unknown→allow, anonymous→/login, non-admin→/dashboard; all 23 admin routes use `beforeLoad: createAdminGuard(...)`, users.tsx fixed 2026-08-26, `AdminShell` secondary client guard | PARITY (verified rg no JSON.parse(localStorage) in routes, E2E 41/41) |
| ADMIN_COMPLIANCE_ACK_REQUIRED (423) | `adminComplianceStore.requireAcknowledgement(err.metadata)` + dialog with ack phrase | `apiClient` 423 → `admin-compliance-required` CustomEvent + `AdminComplianceDialog` in `AdminShell` + `lib/api/admin/compliance.ts` typed | PARITY — dialog blocks, fetchStatus + accept flow, i18n fallback |
| Feature flags | `adminSettingsStore`: `payment_enabled_cached`, `risk_control_enabled`, `ops_monitoring_enabled` + `AppSidebar` filtered | `lib/featureFlags.ts` + `apiClient` response interceptor caches `*_enabled` from `/admin/settings` & `/admin/ops/overview`, `settings.tsx` useEffect caches, `adminGuard` reads `*_cached===false` → redirect | PARITY — Sidebar/Router/Direct-URL three-layer consistent |
| Simple mode | `restrictedPaths: ['/admin/groups','/admin/subscriptions','/admin/redeem',...]` redirect to dashboard | `auth.ts getIsSimpleMode()` + `adminGuard blockSimpleMode:true` on groups/subscriptions/redeem, `AppSidebar` filters RESTRICTED set | PARITY — unified policy, no raw localStorage per-page |
| Typed API | `frontend/src/api/admin/*` (33 modules) | `frontend-new/src/lib/api/admin/` 19 domain files (accounts/affiliates/announcements/audit/channelMonitor/channels/dashboard/groups/ops/orders/payment/promo/promptAudit/proxies/redeem/risk/settings/subscriptions/usage/compliance) | PARITY |
| Query keys | — | `queryKeys.admin.*` (dashboard/usage/audit/announcements/groups/accounts/channels/proxies/redeem/promo/subscriptions/orders/affiliates/settings/risk/promptAudit/ops) unified, 23 routes use them | PARITY |
| AbortSignal | — | all list/search/filter/pagination/monitor queryFn forward `{signal}` (announcements/proxies/channels/monitor/accounts/dashboard/audit/usage etc) | PARITY (grep verified) |
| Sidebar | Vue: full admin nav + feature-gated items | `AppSidebar.tsx` full 22 admin items + `isAdmin()` hide for normal user + payment/risk/ops/simpleMode filters + Separator grouping | PARITY |
| i18n | Full `zh`/`en` for admin | `zh`/`en` admin namespaces present (overview/accounts/channels/audit/promptAudit/ops/settings), routes use `t(...)` with fallback, AdminCompliance keys via fallback | PARTIAL → PARITY in progress (Groups: Group B-E pending full zh) |
| States | Loading/Empty/Error handled in Vue | All admin pages use `LoadingState`/`Skeleton` + `ErrorState` + `EmptyState`, no `.catch(()=>[])` silent failure (rg verified) | PARITY |
...
- **VERIFIED**: 23 (all admin routes including groups/settings hardened)
- **MIGRATED**: 0
- **PARTIAL**: 0
- **SCAFFOLD_ONLY**: 0
